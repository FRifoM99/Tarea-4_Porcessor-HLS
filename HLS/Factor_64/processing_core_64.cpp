...
void processing_core_64(data_t vectorA[N_ELEMENTS], data_t vectorB[N_ELEMENTS], res_t &result, bool mode) {
    ...
    // --- FACTOR 64 ---
    #pragma HLS ARRAY_PARTITION variable=vectorA cyclic factor=64 dim=1
    #pragma HLS ARRAY_PARTITION variable=vectorB cyclic factor=64 dim=1

    int accumulator = 0;

    loop_64: for (int i = 0; i < N_ELEMENTS; i++) {
