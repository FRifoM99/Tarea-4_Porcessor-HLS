#include "processing_core_64.hpp"
#include <iostream>
#include <fstream>
#include <vector>
#include <string>
...
std::vector<std::vector<int>> read_csv(std::string filename, bool skip_header) {
    ...
    if (!file.is_open()) { std::cerr << "ERROR: No se pudo abrir " << filename << std::endl; exit(1); }
    std::string line;
    ...
    while (std::getline(file, line)) {
        ...
        while (std::getline(ss, cell, ',')) {
            try { row.push_back(std::stoi(cell)); } catch (...) { continue; }
        }
...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "   TESTBENCH: PROCESSING CORE 64        " << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    if (inputs.size() != refs.size()) return 1;
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
        ...
        // --- PRUEBA DOT ---
        res_t hw_dot;
        processing_core_64(vecA, vecB, hw_dot, 0);

        if (hw_dot != (res_t)ref_dot) {
            std::cout << "[FAIL 64] Sample " << i << " DOT. Exp: " << ref_dot << " Got: " << hw_dot << std::endl;
            errors++;
        }

        // --- PRUEBA EUC ---
        res_t hw_euc;
        processing_core_64(vecA, vecB, hw_euc, 1);

        long long diff = std::abs((long long)hw_euc - (long long)ref_euc);
        if (diff > EUC_TOLERANCE) {
            std::cout << "[FAIL 64] Sample " << i << " EUC. Exp: " << ref_euc << " Got: " << hw_euc << " Diff: " << diff << std::endl;
            errors++;
        }
    }

    if (errors == 0) {
        std::cout << "\n*** TEST PASSED (Factor 64) ***" << std::endl;
        return 0;
    } else {
