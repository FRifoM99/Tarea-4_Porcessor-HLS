...
int main() {
    std::cout << "========================================" << std::endl;
    std::cout << "   TESTBENCH: PROCESSING CORE 128       " << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    for (size_t i = 0; i < inputs.size(); i++) {
        ...
        // --- PRUEBA DOT ---
        res_t hw_dot;
        processing_core_128(vecA, vecB, hw_dot, 0);

        if (hw_dot != (res_t)ref_dot) {
            std::cout << "[FAIL 128] Sample " << i << " DOT. Exp: " << ref_dot << " Got: " << hw_dot << std::endl;
            errors++;
        }

        // --- PRUEBA EUC ---
        res_t hw_euc;
        processing_core_128(vecA, vecB, hw_euc, 1);

        long long diff = std::abs((long long)hw_euc - (long long)ref_euc);
        if (diff > EUC_TOLERANCE) {
            std::cout << "[FAIL 128] Sample " << i << " EUC. Exp: " << ref_euc << " Got: " << hw_euc << " Diff: " << diff << std::endl;
            errors++;
        }
    }

    if (errors == 0) {
        std::cout << "\n*** TEST PASSED (Factor 128) ***" << std::endl;
        return 0;
    } else {
