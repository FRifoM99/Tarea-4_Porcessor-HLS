...
void processing_core_16(data_t vectorA[N_ELEMENTS], data_t vectorB[N_ELEMENTS], res_t &result, bool mode) {
    ...
    // --- FACTOR 16 ---
    #pragma HLS ARRAY_PARTITION variable=vectorA cyclic factor=16 dim=1
    #pragma HLS ARRAY_PARTITION variable=vectorB cyclic factor=16 dim=1

    int accumulator = 0;

    loop_16: for (int i = 0; i < N_ELEMENTS; i++) {
